...
```powershell
Get-FileHash .\AutoPierCam-0.2.2-x64.msi -Algorithm SHA256
```

Install with the default sign-in behavior and a verbose MSI log:

```powershell
msiexec.exe /i .\AutoPierCam-0.2.2-x64.msi /qn /norestart /l*v .\autopiercam-install.log
```

Install without the optional sign-in feature:

```powershell
msiexec.exe /i .\AutoPierCam-0.2.2-x64.msi /qn /norestart ADDLOCAL=MainApplication /l*v .\autopiercam-install.log
```
